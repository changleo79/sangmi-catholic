  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-[200] md:hidden">
      <div className="mx-auto max-w-md px-4 pb-4">
        <div className="grid grid-cols-4 gap-2 rounded-3xl bg-white/90 backdrop-blur-xl border border-white shadow-2xl p-2">
          {navItems.map((item) => {
            ...
            return (
              <Link
                key={item.to}
                to={item.to}
                className={`flex flex-col items-center gap-1 py-2 rounded-2xl text-xs font-semibold transition-all duration-200 ${
